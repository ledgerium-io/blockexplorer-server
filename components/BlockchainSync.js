...
const syncProgressBar = new cliProgress.Bar({
...
const setTotalProgressBar = (total) => {
    ...
    Promise.all([this.getLastKnownBlock(), this.getLatestBlock()])
      .then((data) => {
        ...
        if (this.syncing && (lastBlockProcessed < latestBlock)) {
          this.clearConsole();
          this.printSyncing();
          if ( (latestBlock - lastBlockProcessed) > parseInt(process.env.SYNC_REQUESTS,10)) {
            this.batchBlockRequest(lastBlockProcessed, (lastBlockProcessed + parseInt(process.env.SYNC_REQUESTS, 10)) );
          } else {
